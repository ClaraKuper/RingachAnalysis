...
new_data<-read.table('long.format.txt') # the long data format as generated by reshape.data

# reset time to stimulus offset
new_data$time2 <- new_data$telSequenceOn1+round(4*1000/120)-new_data$sacOnset
timesteps <- seq(0,-198,-10) # set the length for each timebin
...
for (vp in participants){
  ...
  for (cond in conditions){
    for (ori in (sort(c(orientations,-0.5)))){ #this is for the orientation of interest
      ...
      if (checkval){
        collect.table[orirow,1]<- -ori
      }
      else {
        collect.table[orirow,1]<-ori #write current orientation to table
      }
      ...
      for (tdv in timesteps){ 
        ...
        for(rows in 1:nrow(part_data)){ 
          if(part_data[rows,27]==ori&part_data[rows,9]==cond){ # orientation
            if(is.element(part_data[rows,46],timewin)) # intially, the time information is set to presentation offset, 
...
collect.c<-cast(collectm2,orientation+condition~variable,mean)
collect.p <- melt(collect.c,id=c("orientation","condition"),measure=paste(timesteps))
con <- ggplot(collect.p, aes(variable, orientation)) +facet_grid(condition~.)+ geom_tile(aes(fill = value))+scale_fill_gradient(low = "black",high = mygold)+mytheme
